...
var conString = "pg://postgres:HaveFun@127.0.0.1:5432/chicago_divvy_stations_status";
var pg_connection_divvy_trips = "pg://postgres:HaveFun@127.0.0.1:5432/chicago_divvy_trips";
var pg_covid = "pg://postgres:HaveFun@127.0.0.1:5432/covid_stat";
...
var pgCovid = new pg.Client(pg_covid);
pgCovid.connect();
...
async function getCovidHeatMapData(query) {
  const resp = await pgCovid.query(query);
  lData = []
  ...
  //console.log(lData);
  covid_data.push(lData);
  return covid_data;
}

router.get('/heatmap', function(req, res, next) {
  const query = {
    text: 'select * from covid_stat_heatmap'
  }
